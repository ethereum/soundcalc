...
\begin{itemize}
    ...
    $$
    \ebatchlinJ = \frac{2^{m_0}}{(2\min(\eta,\sqrt{\rho}/20))^5}
    $$
...
\begin{comment}
\subsection{Folding Error \(\epsilon^{\mathrm{fold}}_{i,s}\)}

Base error (two terms):
\[
  \varepsilon^{\mathrm{fold,base}}_{i,s}
    = d \cdot \frac{\ell_{i,s-1}}{F}
      + \texttt{regime.get\_error\_powers}(R_i, n_{i,s}, \mathbb{F}, 2).
\]
