...
\subsection{Security level for a FRI-based VM}
...
\begin{enumerate}
    \item Calculate the FRI round-by-round soundness errors $\efrirbrU,\efrirbrJ$ using the formula from the next section.
    \item Obtain optimal $\delta_U,\delta_J$ parameters.
    \item Obtain the list sizes $L_U,L_J$.
\end{enumerate}

\subsubsection{RBR soundness in UDR}\label{sec:fri_rbr_udr}

\subsubsection{RBR soundness in UDR}\label{ sec:fri_rbr_jbr}
