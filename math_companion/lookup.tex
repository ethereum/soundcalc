...
If the aggregation of the columns is done with a multivariate polynomial, the probability is then  $\epsilon_{\mathsf{sum}}\leq \frac{(L+T)\log(S)}{F}$.
...
\begin{itemize}
	...
	\item $T$ = rows of $\mathbf{T}$
	\item $L$ = rows of $\mathbf{L}$
	\item $H_L$ = size of alphabet used to interpolate $T$.
	\item $H_T$ = size of alphabet used to interpolate $L$.
	\item $H=H_L+H_T$
	\item $S$ = number of columns of $T$ and $L$
	\item $M$ = number of lookups performed on $\mathbf{T}$
\end{itemize}
...
By the same reasoning as above, we have that a cheating prover can get lucky with $\alpha$ with probability $\epsilon_{\mathsf{sum}}\leq \frac{H}{F}$. 

\subsubsection{Multi column:}

As the tables $\mathbf L, \mathbf T$ are already represented as tensors, considering $S$ columns consists on expanding $\mathbf L, \mathbf T$ with $\log (S)$ new variables. Still, to ensure all rows are lookup at together, those variables should be evaluated at random elements and the probability the prover gets lucky with those randomness is $\epsilon_{\mathsf{sum}}\leq \frac{\log(S) H}{F}$.
...
The soundness error is then bounded as $\epsilon_{\mathsf{sum}}\leq \frac{KH}{F}$.
